...
def query_command(query: str, topic: str = typer.Option(..., help="Topic search"),
                  model: str = typer.Option(..., help="Model (es. word2vec, tfidf, ..."),
                  res_format: str = typer.Option(..., help="Result format (es. documents, answer)")):
    start_time = time.time()
    Query.make_query(query, topic=topic, model=model, generate_text=res_format == 'answer')
    end_time = time.time()
